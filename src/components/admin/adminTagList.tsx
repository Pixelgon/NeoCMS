  ...
  const updateTag = async (tagId: string) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        layoutData?.showToast({
          message: "Tag byl aktualizován",
          type: "success",
        });
      } else {
        layoutData?.showToast({
          message: "Chyba při aktualizaci tagu",
          type: "error",
        });
      }
    } catch (error) {
      ...
      <div className="flex flex-col gap-2">
        {loading ? (
          ...
          tags.map((tag) => {
            ...
            const isChanged =
              editedTags[tag.id] !== undefined &&
              editedTags[tag.id] !== tag.name;
            return (
              <div key={tag.id} className="flex gap-2">
                ...
                <Btn type="button" onClick={() => openDeleteDialog(tag)} prim>
