...
export const AdminPanel: FC = () => {
  ...
  const onSaveAll = async () => {
    layoutData.showDialog({
      message:
        "Opravdu chcete uložit všechny změny bloků? Tato akce nelze vrátit zpět.",
      btnR: {
        text: "Ano",
        onClick: async () => {
          ...
          if (result.ok) {
            layoutData.showToast({
              message: "Všechny bloky byly úspěšně uloženy.",
              type: "success",
            });
          } else {
            layoutData.showToast({
              message: "Některé bloky se nepodařilo uložit. Zkuste to znovu.",
              type: "error",
            });
          }
        },
  ...
  const onResetAll = () => {
    layoutData.showDialog({
      message:
        "Opravdu chcete zahodit všechny změny bloků? Tato akce nelze vrátit zpět.",
      btnR: {
        text: "Ano",
        onClick: () => {
          layoutData.closeDialog();
          resetAll();
          layoutData.showToast({
            message: "Všechny změny bloků byly zahozeny.",
            type: "success",
          });
        },
      },
      ...
      <AnimatePresence mode="popLayout">
        <AdminLink
          ...
          className={
            isProjectUnsaved()
              ? "!text-err"
              : layoutData.activeModalKey === "project"
                ? "!text-prim"
                : ""
          }
          key="project"
        >
          ...
          <span>
            {isProjectUnsaved() ? `Neuložený projekt` : "Nový projekt"}
          </span>
        </AdminLink>
        <AdminLink
          onClick={() => openModal("tagModal")}
          className={
            layoutData.activeModalKey === "tagModal" ? "!text-prim" : ""
          }
          key="tags"
        >
        ...
        <AdminLink
          ...
          className={
            layoutData.activeModalKey === "projectList" ? "!text-prim" : ""
          }
          key="projectList"
        >
