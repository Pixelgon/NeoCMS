...
export const ImageUpload: FC<ImageUploadProps> = ({ id, name, label, required, className, value, onChange }) => {
   ...
   return (
      <div className={`flex flex-col ${className || ""}`}>
         ...
         <input type="file" id={id} name={name} accept="image/*" onChange={handleFileChange} className={`text-white file:text-sec file:bg-pxlgn-gradient file:p-2 file:mr-2 file:rounded-3xl ${photoPreview ? 'my-2' : ''}`} />
