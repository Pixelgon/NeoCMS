...
export const Textarea: FC<TextareaProps> = ({className, id, name, required, label, placeholder, value, onChange}) => {
   ...
   return (
      <div className={`flex flex-col ${className ? className : ''}`}>
            ...
            <textarea value={value} onChange={onChange} name={name} id={id} required={required} placeholder={placeholder} className={'bg-sec p-3 rounded-3xl !outline-none text-wh font-quicksand text-lg relative z-20 w-full border border-prim min-w-full min-h-[104px] max-h-[min(200px,20vh)] resize-y rounded-br-none transition-transform focus-within:bg-modal'}/>
      </div>
   )
}
